...
## 3. System Invariants (Non-Negotiable Requirements)

The following properties are **system invariants** — non-negotiable requirements that every valid allocation must satisfy, regardless of which objective function is selected. These are not tunable parameters or platform objectives; they are preconditions for any allocation to be considered valid.

1. **Constraint adherence**: Every allocation must satisfy all promoter-defined hard constraints without exception. No objective function may override or relax a constraint.

2. **Allocation explainability**: Any allocation must be accompanied by a clear explanation of why it was selected and why alternatives were rejected. Solutions that cannot be explained in plain language are considered invalid, regardless of objective performance.

3. **Deterministic reproducibility given the same inputs**: Given identical fan requests, promoter constraints, and objective function parameters, the allocation engine must produce the same result every time. Non-deterministic allocations undermine auditability and stakeholder trust.

---

## 4. Candidate Objective Functions
...
### 4.1 Maximize Total Fulfillment
...
### 4.2 Maximize Revenue Given Fixed Prices
...
### 4.3 Maximize Weighted Fan Satisfaction
...
### 4.4 Minimize Maximum Unfairness (Egalitarian / Minimax)
...
### 4.5 Hybrid: Pareto-Optimal Frontier
...
## 7. Open Questions for Subsequent Phases
